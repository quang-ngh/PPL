...
class LexerSuite(unittest.TestCase):
    ...
    def test_lexer_200(self):
        ...
        self.assertTrue(TestLexer.test(my_input, my_expected_output, 200))
            ...
            self.assertTrue(TestLexer.test("asfkajsf0", "asfkajsf0,<EOF>", 101))
            self.assertTrue(TestLexer.test("aksks_", "aksks_,<EOF>", 101))
            self.assertTrue(TestLexer.test("_asksk", "_asksk,<EOF>", 101))

            self.assertTrue(TestLexer.test("number_of_tokens", "number_of_tokens,<EOF>" 101))   
            self.assertTrue(TestLexer.test("isChangeAngle", "isChangeAngle,<EOF>"), 101)
            self.assertFalse(TestLexer.test("0123asfaf", "0123asfaf,<EOF>"), 101)
